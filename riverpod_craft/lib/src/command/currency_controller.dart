...
class ConcurrentController<DataT, F extends Object, Arg extends Record>
  ...
  /// Maps a raw caught error into the typed error [F] before it is emitted.
  final F Function(Object error) _mapError;
  ...
  void _setupEventHandler(ActionStrategy strategy) {
    on<_Fire<Arg>>((event, emit) async {
      try {
        emit(ArgCommandState<DataT, F, Arg>.loading(event.arg));
        final data = await _action(event.arg);
        emit(ArgCommandState<DataT, F, Arg>.data(event.arg, data));
      } catch (e) {
        emit(ArgCommandState<DataT, F, Arg>.error(event.arg, _mapError(e)));
      }
